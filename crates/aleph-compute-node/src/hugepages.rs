...
pub fn allocate_hugepages(
    ...
    // When no explicit limit is set, use total RAM — per-node headroom is
    // applied inside compute_2m_budget, so subtracting it here too would
    ...
    if failed_nodes > 0 && failed_nodes == total_nodes {
        anyhow::bail!("hugepage allocation failed on all {failed_nodes} NUMA nodes");
    }
...
mod tests {
    ...
    fn test_allocate_hugepages_full() {
        ...
        // effective_limit = total_ram = 128000 (headroom applied per-node only)
        // per_node_cap = 128000 / 2 = 64000
        // Node 0: cap = min(64000, 64000-4096) = 59904, reserved_1g = 2048, budget = 57856/2 = 28928
        // Node 1: cap = min(64000, 64000-4096) = 59904, reserved_1g = 0, budget = 59904/2 = 29952
