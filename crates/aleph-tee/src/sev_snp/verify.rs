...
pub async fn verify_sev_snp_report(
    ...
    // 5. Enforce VMPL 0 — reject reports from less privileged levels.
    // VMPL 0 is the most privileged (firmware/OVMF). A report from VMPL > 0
    // could be generated by a rogue component and should not be trusted.
    if parsed.inner.vmpl != 0 {
        bail!(
            "attestation report from VMPL {} — only VMPL 0 is accepted",
            parsed.inner.vmpl,
        );
    }
