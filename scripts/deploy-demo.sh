#!/usr/bin/env bash
#
# Deploy and run the CVM demo on a remote SEV-SNP server.
#
# Usage: ./scripts/deploy-demo.sh <ssh-host> [--amd-product Genoa] [--keep-bridge]
#
# Builds artifacts locally, copies them to the remote, and runs demo.sh via SSH.
...
usage() {
    ...
    echo "  --amd-product     AMD product name (default: Genoa). Forwarded to demo.sh"
    echo "  --keep-bridge     Don't remove bridge on exit. Forwarded to demo.sh"
    exit 1
}
...
info "Running demo.sh on ${REMOTE}..."
echo
ssh -t "$REMOTE" "sudo ${REMOTE_DIR}/demo.sh ${REMOTE_DIR} ${DEMO_ARGS[*]+"${DEMO_ARGS[*]}"}"
